...
xy1_points = np.zeros((15,2))
#y_points = np.zeros(30)
...
for i in range(15):
    ...
    point = [x,y]
    #print(point)
    #np.append(xy_points,[point],axis = 0)
    xy1_points[i] = point
...
for i in range(15):
    ...
    r = circle_r2
    # calculating coordinates
    x = r * math.cos(alpha) + circle_x2
    y = r * math.sin(alpha) + circle_y2
    point = [x,y]
    #print(point)
    #np.append(xy_points,[point],axis = 0)
    xy2_points[i] = point
...
X = np.zeros((30,2))
X[0:15,0] = xy1_points[:,0]
X[0:15,1] = xy1_points[:,1]
X[15:,0] = xy2_points[:,0]
X[15:,1] = xy2_points[:,1]
...
plt.scatter(points1_x,points1_y,c = 'r')
plt.scatter(points2_x,points2_y,c = 'b')
#plt.show()
#print("Random point", xy_points)
...
clf_rbf = svm.SVC(C = param_C_rbf, kernel = 'poly', degree = 2)
